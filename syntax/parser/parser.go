...
// uia
// Basic Visit methods for simple rules
func (v *ASTBuilder) VisitArrayNotation(ctx *ArrayNotationContext) interface{} {
    // This is called by the parent rules that need array notation
    // The sizeExpr is already handled in VisitDeclaration and VisitParameter
    return v.Visit(ctx.Expression())
}

func (v *ASTBuilder) VisitType(ctx *TypeContext) interface{} {
    // The type name is already extracted using GetText() in parent methods
    return ctx.GetText()
}

func (v *ASTBuilder) VisitDeclarationRemainder(ctx *DeclarationRemainderContext) interface{} {
    // already processed in VisitDeclaration
    return nil
}

func (v *ASTBuilder) VisitElseClause(ctx *ElseClauseContext) interface{} {
    // already processed in VisitIfStatement by checking for a Block or IfStatement
    return nil
}

func (v *ASTBuilder) VisitAssignmentRest(ctx *AssignmentRestContext) interface{} {
    return v.Visit(ctx.AssignmentExpression())
}
...
func (v *ASTBuilder) VisitLogicalOrRest(ctx *LogicalOrRestContext) interface{} {
    return v.Visit(ctx.LogicalAndExpression())
}

func (v *ASTBuilder) VisitLogicalAndRest(ctx *LogicalAndRestContext) interface{} {
    return v.Visit(ctx.EqualityExpression())
}

// Equality and comparison components
func (v *ASTBuilder) VisitEqualityRest(ctx *EqualityRestContext) interface{} {
    return v.Visit(ctx.ComparisonExpression())
}

func (v *ASTBuilder) VisitEqualityOperator(ctx *EqualityOperatorContext) interface{} {
    return ctx.GetText()
}

func (v *ASTBuilder) VisitComparisonRest(ctx *ComparisonRestContext) interface{} {
...
func (v *ASTBuilder) VisitAdditionExpressionRest(ctx *AdditionExpressionRestContext) interface{} {
    return v.Visit(ctx.MultiplicationExpression())
}

func (v *ASTBuilder) VisitAddSubtractOperator(ctx *AddSubtractOperatorContext) interface{} {
    return ctx.GetText()
}

func (v *ASTBuilder) VisitMultiplicationExpressionRest(ctx *MultiplicationExpressionRestContext) interface{} {
    return v.Visit(ctx.UnaryExpression())
}

func (v *ASTBuilder) VisitMultDivModOperator(ctx *MultDivModOperatorContext) interface{} {
    return ctx.GetText()
}

func (v *ASTBuilder) VisitUnaryOperator(ctx *UnaryOperatorContext) interface{} {
    return ctx.GetText()
}
...
func (v *ASTBuilder) VisitFunctionCallArgs(ctx *FunctionCallArgsContext) interface{} {
    if argList := ctx.ArgList(); argList != nil {
        return v.Visit(argList)
    }
    return []ast.Expression{}
}

func (v *ASTBuilder) VisitArgList(ctx *ArgListContext) interface{} {
    var args []ast.Expression
    
    for _, argCtx := range ctx.AllAssignmentExpression() {
        arg := v.Visit(argCtx).(ast.Expression)
